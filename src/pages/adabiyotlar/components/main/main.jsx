...
export function MainAdabiyot() {
  ...
  const {
    ...
    isPending,
    error,
  } = useGetFetch(`${import.meta.env.VITE_BASE_URL}/edu-prof/adabiyotlar/?category=${category}&page=${page}&search=${search}`);
            ...
            <div className="booksBlok">
              {adabiyotlar.results?.length == 0 ? (
                ...
                adabiyotlar.results?.map((item) => (
                  <Link to={`/Adabiyotlar/${item.id}`} key={item.id} className="cardss">
