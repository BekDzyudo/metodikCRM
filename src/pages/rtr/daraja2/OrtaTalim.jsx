...
function OrtaTalim() {
  ...
  const { data, isPending, error } = useGetFetch(
    `https://rtr.profedu.uz/api/v1/rtr_base_app/subject-list/2/level/?general_subject=${category}&page=${page}`
  );
    ...
    <div className="container">
      ...
      {sidebar && (
          <div className="sidebar">
            ...
            {sidebar &&
              sidebar[1]?.general_subjects.map((item, index) => {
                return (
                  <button
        ...
        <div className="content">
          ...
          {data?.results.length ? (
            data.results.map((item) => {
              return (
                <Link to={`/raqamli-talim-resurslari/orta-professional-talim/${item.id}`} className="cardd" key={item.id}>
                  <div className="image">
